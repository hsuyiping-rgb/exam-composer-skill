...
- 📌 **兩點留給命題教師判斷（非錯誤，是設計選擇）**：(a) 第 22 題折線圖已預先畫上星期一(12)、星期二(8) 兩點，學生從第三點接續——若原意是整張空白由學生自畫，需重繪該圖；(b) 第 1 頁選擇題只排到第 9 題、第 7 頁只剩 27-(3)，版面留白偏多，作答空間充足但若想壓成 6 頁可再調整。LibreOffice 字型替換與 Word 不同，仍建議教師用 Word 開一次確認。
...
1. **請命題教師用 Word 開啟正式試題卷做最後確認**，特別是上方「兩點留給命題教師判斷」的第 22 題折線圖預先畫點問題。這是本任務唯一還沒收尾的事。
